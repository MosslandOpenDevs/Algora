...
### Changed
- **`LOCAL_LLM_NUM_CTX` raised 8192 → 16384 to match MOSS.AO on the shared Ollama host** — MOSS.AO asked us to converge because the host serves **one model instance at a time**, so a call at any `num_ctx` other than the resident one forces a full unload/reload; with two services picking different numbers every alternation pays for it. Re-measuring the host directly (2026-08-06, Ollama 0.32.5) confirmed the mechanism but corrected three of the figures the request was built on: a cold load is **4.3–4.5s, not ~60s**; a call at a non-resident `num_ctx` **completes after that reload rather than hanging** (their reported 40s timeouts did not reproduce — a 3,170-token/16384 request returned in 11.2s while our production cadence ran alongside it, and only one of six of our calls paid a 4.45s reload); and 16384 costs us **nothing in VRAM** — gemma3:4b reports 2.89 GB resident at 16384 vs 3.03 GB at 8192, since sliding-window attention keeps the KV cache flat. Decode time scales with tokens emitted (ours are 19–31), not with the window, and our largest real prompt is ~4.5k tokens, so this is a coordination value and not a capacity need — documented as such in `.env.example` and at both use sites so it is not changed unilaterally again. The stale `keep_alive` comment claiming "cold loads cost ~60s" was corrected to the measured figure. Shortening `keep_alive` was **declined with measurements** — staying resident is what keeps us from re-loading in front of MOSS.AO, since the host keeps one runner per model name. (Their other suggestion, routing our small `fast` calls to a 1B model, was declined on the same reasoning and that reasoning was wrong — see the correction under Added below.) Separately, polling `/api/ps` under the fixed config caught a **third** `num_ctx` in rotation (4096) that the coordination request did not account for: `IdeaScorer.SCORING_NUM_CTX = 4096` in MOSS.AO's own scoring path, a hard-coded per-call override added as a workaround for this very problem, which was still evicting the shared 16384 runner on every idea-triage item. **Resolved** — MOSS.AO confirmed all four corrections and removed the override (`e0468d5`, verified as an ancestor of their deployed HEAD and pinned by `assert not hasattr(IdeaScorer, "SCORING_NUM_CTX")`); the host has since held one `gemma3:4b` at `ctx=16384` across continuous polling with no flips.
...
### Added
- **`OLLAMA_NUM_PARALLEL` raised 1 → 2 on the shared host** (`docs/ollama-shared-host-tuning.md`, with Korean translation) — with the `num_ctx` split resolved on both sides, the remaining limit was serialization *within the shared `gemma3:4b` runner*: a trivial call fired 8s into a 16.5s generation returned at 16.6s having simply waited, and Algora's shape (2 calls/min, 19–31 output tokens, sub-second) and MOSS.AO's (multi-thousand-token generations, tens of seconds) blocked each other in both directions. Measured live at each setting, bracketed by single-request controls: **1 → ~107 tok/s aggregate (2.89 GB), 2 → 173 tok/s (2.98 GB), 4 → 235 tok/s (3.30 GB)**, with single-request decode unchanged throughout against the ~109 tok/s pre-change baseline. **2 was chosen over 4** because `MAX_LOADED_MODELS` is 2 and `NUM_PARALLEL` applies per runner, so 4 would let two resident models reserve 8 slots between them; 2 keeps ~74% of the throughput gain for ~22% of the VRAM cost. The headroom is real rather than theoretical — an unrelated `gemma3:1b` from another client on the LAN appears on this host periodically, and with both resident the total is 3.92 GB of ~5.9 GiB usable, serving concurrently (a 1b probe fired 2s into a 4b generation returned in 0.38s, no eviction). Flash attention and a `q8_0` KV cache were then enabled on the host — suggested here as a likely win — and re-measured with the same controls: **no measurable difference on any axis** (single 104.7/107.2 tok/s, aggregate 229 vs 235 tok/s, VRAM identical at 3.30 GB). Recorded as a null result rather than dropped, with the likely reason: gemma3's sliding-window attention already keeps the KV cache small, so quantising it saves nothing worth measuring.
  - **False alarm recorded rather than quietly dropped:** mid-change sampling showed decode at 5.2 tok/s and prefill at 101 tok/s and was reported as a catastrophic regression, with a Vulkan-vs-CUDA backend switch as the suspected cause. The host log disproved that (`library=CUDA` throughout) and controlled re-measurement showed no regression at all — the samples were contaminated, because once `NUM_PARALLEL > 1` Algora's own production traffic genuinely shares the GPU, so an uncontrolled "single request" reading is really measuring *n*-way contention. The doc now prescribes bracketing every concurrency measurement with single-request controls, since this host is never idle.
  - **Consequently not doing:** moving the chatter scheduler to `gemma3:1b`. It was the fallback for head-of-line blocking; with the host serving concurrent requests there is no blocking left to fix, so it would trade user-visible chatter quality for nothing.
  - **Correction to an earlier claim in this entry's sibling above:** "the host serves one instance at a time" was generalised from same-model tests and is too broad. `ollama ps` on the box showed `gemma3:4b` and `gemma3:1b` resident together, and a `gemma3:1b` call fired 8s into a 17.3s `gemma3:4b` generation returned in **0.59s** — distinct models get distinct runners and run in parallel. Only same-model/different-`num_ctx` thrashes, and only same-runner requests serialize. The `num_ctx` convergence is unaffected; what changes is that MOSS.AO's suggestion of a smaller model for our high-frequency path was right in principle and our rejection of it was wrong.
