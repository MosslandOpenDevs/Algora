...
- **Monthly reports did not state the period they covered** — the weekly
  ...
  window its figures came from. That gap surfaced the moment the corrections
  below were attempted: `regenerateReport` refused both monthly reports rather
  than infer a window, which was the guard working. Monthly reports now state
  their period too, and the correction path accepts an explicit `period` for
  the two published before they did — the caller supplies the window and owns
  that claim, while the default path still refuses to invent one.

### Added
- **Published reports can be corrected in place** (`regenerateReport`) — the 12
  reports published before the fix below all state `LLM API Calls | 0` and
  `Uptime | 99.9%`, figures that were returned as constants rather than
  measured. Recomputing one needs its original period, and that is taken from
  the report itself: the body carries `**Period:** Jun 15, 2026 - Jun 22, 2026`.
  A report that does not state one is refused rather than have its window
  guessed from the publication date — guessing would be the same class of
  mistake being corrected. Every regenerated body opens with a correction
  notice naming what was wrong, because quietly replacing a published
  governance record would be its own failure whatever the numbers say
  afterwards. `dryRun` computes without writing, so a correction can be read
  before it lands.
