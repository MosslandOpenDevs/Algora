...
### Added
- **Joint request to the Ollama host admin for `OLLAMA_NUM_PARALLEL=2`** (`docs/ollama-host-parallelism-request.md`, with Korean translation) — with the `num_ctx` split resolved on both sides, the remaining shared-host limit is serialization, which neither project can configure: the host serves one request at a time, so a trivial call fired 8s into a 16.5s generation returned at 16.6s having simply waited. Algora's shape (2 calls/min, 19–31 output tokens, sub-second) and MOSS.AO's (multi-thousand-token generations, tens of seconds) block each other in both directions. The request carries the measured cost of the change: two slots at `num_ctx=16384` allocate the KV of a 32768 window, which resident on this host is **2.90 GB vs 2.89 GB** — ~10 MB, because gemma3's sliding-window attention keeps the cache near-flat in context length. It explicitly asks to leave `OLLAMA_MAX_LOADED_MODELS` at 1, since a second loaded model is the thrash we just removed.
