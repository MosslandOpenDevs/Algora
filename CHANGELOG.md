...
### Added
- **Request to the Ollama host for `OLLAMA_NUM_PARALLEL=2`** (`docs/ollama-host-parallelism-request.md`, with Korean translation) — with the `num_ctx` split resolved on both sides, the remaining shared-host limit is serialization *within the shared `gemma3:4b` runner*, which neither project can configure: a trivial call fired 8s into a 16.5s generation returned at 16.6s having simply waited. Algora's shape (2 calls/min, 19–31 output tokens, sub-second) and MOSS.AO's (multi-thousand-token generations, tens of seconds) block each other in both directions. The request carries the measured cost: two slots at `num_ctx=16384` allocate the KV of a 32768 window, which resident here is **2.90 GB vs 2.89 GB** — ~10 MB, because gemma3's sliding-window attention keeps the cache near-flat in context length. Verified against the host's own startup config (`OLLAMA_NUM_PARALLEL:1`, unset in the environment and therefore just the default; `MAX_LOADED_MODELS:0`/auto, which the request deliberately leaves alone) and its GPU probe (RTX 5060, 8.0 GiB total / 6.9 GiB available, ~3.8 GiB resident).
