...
### Added
- **`OLLAMA_NUM_PARALLEL` raised 1 → 4 on the shared host** (`docs/ollama-shared-host-tuning.md`, with Korean translation) — with the `num_ctx` split resolved on both sides, the remaining limit was serialization *within the shared `gemma3:4b` runner*: a trivial call fired 8s into a 16.5s generation returned at 16.6s having simply waited, and Algora's shape (2 calls/min, 19–31 output tokens, sub-second) and MOSS.AO's (multi-thousand-token generations, tens of seconds) blocked each other in both directions. Measured after the change, bracketed by single-request controls: **235 tok/s aggregate across 4 concurrent requests (2.2x), with single-request decode unchanged at 106.6 → 106.9 tok/s** against the ~109 tok/s pre-change baseline, for **+0.41 GB VRAM** (2.89 → 3.30 GB against 6.9 GiB available on the RTX 5060). Per-request latency under full 4-way load rises ~1.8x, which is the intended trade. Flash attention and a `q8_0` KV cache were then enabled on the host — suggested here as a likely win — and re-measured with the same controls: **no measurable difference on any axis** (single 104.7/107.2 tok/s, aggregate 229 vs 235 tok/s, VRAM identical at 3.30 GB). Recorded as a null result rather than dropped, with the likely reason: gemma3's sliding-window attention already keeps the KV cache small, so quantising it saves nothing worth measuring.
