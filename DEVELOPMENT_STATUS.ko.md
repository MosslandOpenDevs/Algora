...
**배포 메모**: 프로덕션 호스트(atrn-vm-linux, pm2 `algora-api`)에서
빌드·재시작해야 효력이 생깁니다.
...
**해결: 호스트에 `OLLAMA_NUM_PARALLEL=2` 적용.** 직렬화가 사라졌고 단일 요청
decode는 그대로입니다. 각 설정을 단일 요청 대조군 사이에 끼워 측정했습니다:

| `NUM_PARALLEL` | `gemma3:4b` VRAM | 동시 4개 집계 |
|---|---|---|
| 1 | 2.89 GB | 약 107 tok/s |
| **2** (채택) | **2.98 GB** | **173 tok/s** |
| 4 | 3.30 GB | 235 tok/s |

4가 아닌 2인 이유는 `MAX_LOADED_MODELS`가 2이고 `NUM_PARALLEL`이 러너마다
적용되기 때문입니다 — 4라면 상주 모델 둘이 도합 슬롯 8개를 잡습니다. 2는
이득의 약 74%를 VRAM 비용 약 22%로 확보하면서, LAN의 다른 클라이언트가
주기적으로 올리는 `gemma3:1b`의 자리를 남깁니다. 둘 다 상주 시 합계는 가용
약 5.9 GiB 중 3.92 GB이며 동시에 서빙됩니다. 전체 정리는
...
대한 대비책이었는데 `NUM_PARALLEL=2`로 차단 자체가 사라졌으므로, 지금
