...
## 최근 작업: 컬렉터 재시작 루프 수정 (2026-08-14)

프로덕션 상태 점검에서 시그널 컬렉터 4개 전부가 30초마다 영구
재시작되는 루프가 발견됐습니다(컬렉터당 누적 재시작 6.5만–12.8만 회,
CoinGecko는 상시 HTTP 429 응답, 활동 피드의 1/3이 재시작 스팸). 원인:
헬스체크가 "최근 5분 내 `signals` 테이블 신규 행"으로 생존을 판정했지만,
시그널은 삽입 시 중복 제거되고 fetch 주기는 15–120분이라 정상 컬렉터가
항상 죽은 것으로 읽혔습니다. 재시작마다 `start()`가 전체 소스 즉시
수집을 발화시켜 실제 수집은 설정보다 ~17배 잦았고, 주기 타이머는 한
번도 발화할 만큼 살아남지 못했습니다.

`apps/api/src/services/collectors/index.ts`에서 수정: 생존 판정은 이제
소스 테이블의 `last_fetched`(성공한 fetch마다 기록됨)를 컬렉터별 가장
느린 설정 주기에서 유도한 윈도(2주기, 최소 10분, 최대 6시간)와
비교합니다. 한 번도 fetch 못 한 컬렉터는 서비스 시작 시점부터 측정하고,
백오프 대기 중인 재시작은 중복 큐잉되지 않으며, 백오프 지수는
`2^누적횟수`(이미 `Infinity`로 오버플로)가 아니라 마지막 성공 이후
연속 재시작 횟수로 상승합니다. 루프 시대의 오염된 재시작 카운트
(>10,000)는 로드 시 1회 정리됩니다. `/api/pipeline/alerts`의 자체 고정
5분 윈도도 같은 판정(`getStaleness()`)을 읽도록 통일했습니다. 아울러
정상 `completeSession()` 경로에 `concluded_at`을 기록해 파이프라인
헬스의 `completedLast24h`가 0으로 나오던 문제도 고쳤습니다. 테스트:
`collector-health.test.ts` 11건이 실패 모드를 고정하고, 프로덕션 DB
사본 대상 검증에서 새 로직이 4개 컬렉터 모두 healthy로 판정했습니다.

**배포 메모**: 비공개 프로덕션 애플리케이션 호스트에서 빌드한 뒤
pm2 `algora-api` 프로세스를 재시작해야 효력이 생깁니다.

---

## 이전 작업: 공유 Ollama `num_ctx` 조율 (2026-08-06)

통제된 공유 서비스 시험에서 같은 모델을 서로 다른 컨텍스트 크기로
요청하면 상주 러너가 교체되는 것을 확인해, 참고
`LOCAL_LLM_NUM_CTX`를 `16384`로 올렸습니다. 해당 모델을 쓰는 모든
클라이언트를 같은 값으로 맞추고, 호출별로 달랐던 오버라이드를
제거했습니다.

재사용 가능한 결론은 다음과 같습니다.

- 같은 모델 + 다른 `num_ctx`: 상주 러너 교체 및 재로드
- 같은 러너에 동시 요청: 설정된 슬롯 공유
- 서로 다른 모델: 용량이 허용되면 별도 러너에서 병렬 실행
- 참고 `NUM_PARALLEL=2` 측정은 단일 슬롯 기준선 대비 상주 메모리
  약 `1.03x`에서 집계 처리량 약 `1.6x`; 배포마다 재측정 필요
- 백그라운드 트래픽을 설정 회귀로 오인하지 않도록 동시성 시험 전후에
  단일 요청 대조군 측정

`16384`는 조율용 기본값이지 실제 호스트 용량을 설명하는 값이 아닙니다.
한정된 프롬프트 요구량을 측정한 뒤 같은 모델을 쓰는 모든 클라이언트에
동시에 변경해야 합니다. 운영 엔드포인트, 호스트명, 테넌트, OS/GPU,
드라이버, 절대 용량 및 원시 벤치마크 정보는 공개 저장소 밖에
보관합니다. 일반화된 절차는 `docs/ollama-shared-host-tuning.ko.md`를
참고하세요.
...
운영자가 숙의를 유료 API로 옮겨야 하는지 물었습니다. 통제된 분석 결과,
눈에 보이던 품질 문제는 로컬 모델이 아니라 애플리케이션 제한값에서
비롯됐습니다.

다른 배포 사례에서 재사용 가능한 규칙 두 가지를 확인했습니다. 서버
기본값이 긴 프롬프트를 조용히 자를 수 있으므로 `num_ctx`를 항상 명시하고,
공유 호스트의 지연·용량 측정을 다른 배포에 일반화하지 않아야 합니다.
Algora 프롬프트는 코드에서 상한이 정해져 있고 대표 시험에서 설정된
컨텍스트 창 아래에 머뭅니다. 정확한 운영 워크로드, 테넌트 정보, 호스트
시간 및 원시 로그는 비공개 운영 기록에 보관합니다.
...
- [x] **pm2 프로세스 관리**
  ...
  - [x] 비공개 애플리케이션 호스트 배포(주소 생략)
  - [x] API 및 web 서비스는 비공개 리버스 프록시 경계 뒤에 바인딩
  - [x] 메모리 제한이 있는 자동 재시작 설정
- [x] **nginx 리버스 프록시**
  - [x] nginx 기반 공개 엣지 프록시(제공자·주소 생략)
