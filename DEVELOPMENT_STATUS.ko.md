...
**남은 것 — 서로 독립적인 두 지렛대, 둘 다 저렴합니다:**

1. **호스트에 `OLLAMA_NUM_PARALLEL=2`** (`setx` 후 Ollama 재시작). 실측
   비용 약 10 MB. 16384 슬롯 2개가 32768 창 분량 KV를 할당하는데
   2.90 GB 대 2.89 GB이기 때문입니다. 두 서비스의 동일 모델 경합을 한
   번에 해소합니다. 정리는 `docs/ollama-host-parallelism-request.md`.
2. **chatter 스케줄러를 `gemma3:1b`로 이동** — Algora 쪽 변경이라 관리자
   승인 불필요. 차단을 일으키는 분당 2회 상시 주기가 바로 chatter이고,
   품질 민감도가 가장 낮은 트래픽입니다(`maxTokens: 100`, 200자 캡, 실패
   시 정적 폴백). 자체 러너를 받아 MOSS.AO와의 대기열 경합에서 완전히
   빠집니다. 단, `complexity: 'fast'` 전체에 적용하면 **안 됩니다**: 7개
   `fast` 호출처 중 4개가 `agora.ts`의 실제 숙의 경로이고 거기서는 모델
   품질이 사용자에게 그대로 노출됩니다. chatter 텍스트 품질에 대한 결정이
   필요합니다.
