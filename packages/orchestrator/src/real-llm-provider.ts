...
export interface RealLLMProviderConfig {
  ...
  /** Default model for Ollama (default: qwen3.5:9b) */
...
export const DEFAULT_REAL_LLM_PROVIDER_CONFIG: RealLLMProviderConfig = {
  ollamaDefaultModel: process.env.LOCAL_LLM_MODEL_CHATTER || 'qwen3.5:9b',
...
export class RealLLMProvider implements LLMProvider {
  ...
  private async generateWithOllama(options: {
    ...
    const model = this.config.ollamaDefaultModel || 'qwen3.5:9b';
 ...
 *
 * The shared remote Ollama only carries qwen3.5:9b at Tier 1, so the local
